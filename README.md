...
# 策略简述
**混合加密策略（hybrid-encryption）**，即由客户端生成一个随机密钥(cek)，以该密钥对称加密消息内容(content)，继而以（与服务端事先约定的）不对称加密方式加密cek，并将加密后的cek与content一并发往服务端即可。该加密方式能够兼顾了不对称加密的安全性和对称加密的高效性，只须保证客户端持有的公钥不被替换（公钥不怕泄露）以及服务端持有的对应私钥没有泄漏，即可非常可靠地保障通信安全。

本产品即提供一套非常简洁工具来帮助您完成该策略的应用，仅需少量配置即可完成全套体系的搭建
